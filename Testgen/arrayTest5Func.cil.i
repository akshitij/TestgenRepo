...
#pragma merger("0","./arrayTest5Func.i","-g,-g")
void foo2(int *foo2_c , int foo2_m , int foo2_l )
{
  char *symName ;
  void *addr ;
  char in[15] ;
  ...
  {
  *(foo2_c + foo2_l) = *(foo2_c + foo2_l) * 3 + 1;
  addEntryToVariableTable("*(foo2_c + foo2_l)", foo2_l);
  add_entryToArraySTable("foo2_c", foo2_l, "foo2_c10", foo2_c + foo2_l, foo2_c + foo2_l,
                         1);
  handleArraySymbolically("foo2_c", foo2_l, "(+ (* *(foo2_c + foo2_l) 3) 1)", foo2_c + foo2_l,
                          foo2_c + foo2_l, 1);
  return;
}
}
...
void createSidTable(void)
{
  ...
  {
  add_condition(2, "(= *(b + k) 7)", "(not (= *(b + k) 7))", 0, 0);
}
}
struct arguments {
   int *b ;
...
int foo1(int *b , int n )
{
  ...
  sprintf(__cil_tmp9, "\t%d\t%d\t%d\t%d\t%d\t%d\n", b[0], b[1], b[2], b[3], b[4],
          n);
  printTestCase("arrayTest5Func_foo1_1435524739.tc", __cil_tmp9);
  add_entryToSTable("n", "s1", & n, & n, 1);
  add_entryToArraySTable("b", 0, "b_0", b, b, 1);
  n = 10;
  add_entryToSTable("n", "Constant", & n, & n, 1);
  k = 5;
  add_entryToSTable("k", "Constant", & k, & k, 1);
  printf((char const * __restrict )"----------------------------------EXECUTION START------------------------\n");
  funcEntry("(int *,foo2_c,array,b)#(int,foo2_m,variable,n)#(int,foo2_l,variable,k)",
            "", "foo2");
  foo2(b, n, k);
  funcExit();
  {
  ...
  add_entryToArraySTable("b", k, "b11", b + k, b + k, 1);
  handleAssignmentSymbolically("exp_outcome", "(= *(b + k) 7)", & exp_outcome, & exp_outcome,
                               1);
  overall_outcome = (int )getConditionalOutcome(2, exp_outcome);
  if (overall_outcome) {
    addEntryToVariableTable("*(b + k)", k);
    add_entryToArraySTable("b", k, "b10", b + k, b + k, 1);
    setBranchInfo(2, 1, 0);
    setTrueExpr(2, "(= *(b + k) 7)");
    setFalseExpr(2, "(not (= *(b + k) 7))");
    addToTree(2, 1, "(= *(b + k) 7)", "(not (= *(b + k) 7))", 0, 1);
    delete_allVariableTableEntry();
    j = 1;
    ...
    setTrueExpr(2, "(= *(b + k) 7)");
    setFalseExpr(2, "(not (= *(b + k) 7))");
    addToTree(2, 1, "(= *(b + k) 7)", "(not (= *(b + k) 7))", 0, 0);
    delete_allVariableTableEntry();
    j = -1;
    add_entryToSTable("j", "Constant", & j, & j, 1);
  }
