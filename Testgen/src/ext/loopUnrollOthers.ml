...
let loopUnroll (f: file) (funName:string) : unit =
  ...
  let doGlobal = function
    ...
    | _ -> ()
  in
  Stats.time "loopUnrollOthers" (iterGlobals f) doGlobal


  
let iterLoopUnrollOthers  (f: file) : unit =
...
let feature : featureDescr = {
  fd_name = "loopUnrollOthers";
  fd_enabled = ref false;
  fd_description = "loop unroll for other than input functions";
  fd_extraopt = [];
  fd_doit = iterLoopUnrollOthers;
  fd_post_check = true
} 
