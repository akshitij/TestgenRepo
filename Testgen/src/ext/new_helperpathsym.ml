...
let makeCreateSTablefxn (fname: string) : varinfo = 
        let v = makeGlobalVar ("createSidTableOf_" ^ fname) 
                    (TFun(voidType, Some [],
                             true, [])) in
         v


let mkcreatetablefxn (fname: string) : instr = 
  let p: varinfo = (makeCreateSTablefxn fname) in
        Call(None, Lval(var p), [], !currentLoc)
...
let makecreateCDGFunction (fname: string) : varinfo = 
        let v = makeGlobalVar ("createCDG_" ^ fname)
                    (TFun(voidType, Some [],
                             true, [])) in
         v

let mkCDGcreatefxn (fname: string) : instr = 
  let i: varinfo = (makecreateCDGFunction fname) in 
...
let helperpathsym (f: file) (fname: string) : unit =
...
let iterHelperPathSymbol (f: file) : unit =
  List.iter (
           fun funcName ->
           if funcName <> "main" then begin
             helperpathsym f funcName;
             (*resetGlobalValues ();*)
             end
         ) !Param.func_list;
  List.iter (fun g ->
