...
let funcrt : typ list ref = ref []

let generateIfStmt () : stmt =
  if (List.length !programFlagVi > 0) && (List.length !funcrt > 0) then begin
 	 E.log "length is positive\n";
 	 let cond = BinOp(Eq, (v2e (List.nth !programFlagVi 0)), integer 1, intType) in
 	 mkStmt (If(cond,(mkBlock [mkStmt (Return(Some(CastE((List.nth !funcrt 0),integer 0)),locUnknown))]),(mkBlock []),locUnknown))
  end
  else begin
 	 E.log "length is 0\n";
 	 mkEmptyStmt ()
  ...
  List.iter (fun st ->
	     ...
	     |Instr (iList) ->
	   	List.iter (fun inst ->	
				...
				if (List.mem fnam !Param.func_list || fnam = "testgen_exit") then begin
	                        	(*funcSig := [] ; 
  ...
  List.iter (fun g ->
	     ...
	     |GFun (fd, loc) ->
		  ...
		  funcrt := !funcrt @ [rt];
	       ...
	       if fd.svar.vname = !Param.func then begin
	          E.log "programFlag = 0 in %s\n" !Param.func;
