...
#pragma merger("0","src/src/loopTest.i","-g,-g")
...
void createSidTable(void)
{
  ...
  {
  add_condition(2, "(<= i 2)", "(not (<= i 2))", 0, 0);
...
int main1(int i )
{
  ...
  printTestCase("loopTest_main1_1432992388.tc", __cil_tmp7);
  add_entryToSTable("i", "s0", & i, & i, 1);

  {
  exp_outcome = i <= 2;
  handleAssignmentSymbolically("exp_outcome", "(<= i 2)", & exp_outcome, & exp_outcome,
                               1);
  overall_outcome = (int )getConditionalOutcome(2, exp_outcome);
  if (overall_outcome) {
    setBranchInfo(2, 1, 0);
    setTrueExpr(2, "(<= i 2)");
    setFalseExpr(2, "(not (<= i 2))");
    addToTree(2, 1, "(<= i 2)", "(not (<= i 2))", 0, 1);
    delete_allVariableTableEntry();
    j = 3;
    ...
    setTrueExpr(2, "(<= i 2)");
    setFalseExpr(2, "(not (<= i 2))");
    addToTree(2, 1, "(<= i 2)", "(not (<= i 2))", 0, 0);
    delete_allVariableTableEntry();
    k = i * 10;
    handleAssignmentSymbolically("k", "(* i 10)", & k, & k, 1);
  }
  }
  {
  exp_outcome = k > 100;
