...
int isNotQueueEmpty();
# 2 "src/src/levelTree.c" 2

struct treeNode *tree_nodes[100] = { ((void *)0) };
...
int ifFirstChild(struct treeNode *node) {
  if (getFchild(getNodeParent(node)) == ((void *)0))
    return 1;
...
struct treeNode *newtreeNode(int sid, int level, char *texp, char *fexp, int outcome) {
  ...
  new_treeNode->fchild = ((void *)0);
  new_treeNode->parent = ((void *)0);
  new_treeNode->next = ((void *)0);
...
void addToTree(int sid, int level, char *texp, char *fexp, int pid, int outcome) {
  ...
  if (getFirstNodeAtLevel(level) == ((void *)0))
  ...
  if (getNodeParent(new_treeNode) != ((void *)0)) {
    if (ifFirstChild(new_treeNode))
