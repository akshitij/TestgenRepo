...
CILLY_LIBRARY_MODULES = pretty inthash errormsg alpha trace stats util clist \
	...
	test param handleBreak handleAssert testgenStub handleCalls insertTestDriver handleExp modifyVars loopUnroll extractStruct makeCDG loopUnrollOthers sidTable sym level_struct vartype varUpdated helperpathsym pathSymbol handleUnseenNodes insertReturnHolders instMapConcolic entryExit fixMainForGlobals\
              testcil\
	      $(CILLY_FEATURES) \
...
odoc: $(OBJDIR)/pretty.cmi $(OBJDIR)/cil.cmi
	...
	ocamldoc -d doc/html/cil/api -v -stars \
             -html \
             -t "CIL API Documentation (version 1.5.1)" \
...
# Create the version document
	cd doc/html/cil; echo "\\\\def\\\\cilversion{1.5.1}" >cil.version.tex
...
pdfdoc: doc/cilpp.tex $(OBJDIR)/pretty.cmi $(OBJDIR)/cil.cmi
	cd doc; echo "\\\\def\\\\cilversion{1.5.1}" >cil.version.tex
