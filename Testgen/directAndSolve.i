...
int getOutputFromConstraintSolver()
{
  char *token, *save;
  ...
  strcpy(result, "\0");
  while (!feof(pipe)) {
    if (fgets(buffer, 1000, pipe) != ((void *)0))
  ...
  if (((void *)0) != token && strcmp("unsat", token) == 0)
    return 0;
...
void directPathConditions() {
  ...
  if (queue.totalElements == 0 || countNoNewConditionAttempts>=10) {
...
# 253 "src/src/directAndSolve.c"
  ...
  *newPathCondition = 0;
  ...
  if (check_position >= 1) {
    if (queue.front->levelptr[check_level + 1] == ((void *)0)) {

      deQueue();
      check_level = 1;
      check_position = 0;

      if(newPathCondition!=((void *)0))
        { newPathCondition = ((void *)0);
  ...
  if (check_level == 1 && check_position == 0)
    {
      while (curr != ((void *)0)) {
        if (curr->outcome) {
    ...
    {
      i = 1;
      while (i < check_position && curr != ((void *)0)) {
        curr = getNextnode(curr);
        i++;
      }

      if ((i < check_position) || (i == check_position && curr == ((void *)0))) {
        check_level++;
        check_position = 1;
      ...
      while (curr != ((void *)0) && curr->parent->sid == parent->sid)
        {
          if (curr->outcome) {
            if (!checkForAllConstants((char *)curr->fexp))
              {strcat(newPathCondition, curr->fexp); strcat(newPathCondition, "##");}
  ...
  if(newPathCondition!=((void *)0))
    { newPathCondition = ((void *)0); free(newPathCondition);}
