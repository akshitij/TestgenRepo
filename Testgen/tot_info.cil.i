...
extern __attribute__((__nothrow__)) char *( __attribute__((__nonnull__(1), __leaf__)) strchr)(char const *__s ,
                                                                                               int __c ) __attribute__((__pure__)) ;
int isWhiteSpace(char const c___0 ) ;
int isOperator(char const c___0 ) ;
int isNotOperator(char const c___0 ) ;
int isAlpha(char const c___0 ) ;
int isDigitDot(char const c___0 ) ;
int isDigit(char const c___0 ) ;
int isWhiteSpace(char const c___0 )
{
  int tmp ;
  ...
  if ((int const )c___0 == 32) {
    tmp = 1;
  } else
  if ((int const )c___0 == 9) {
    tmp = 1;
  } else {
    tmp = 0;
  }
...
int isDigitDot(char const c___0 )
{
  ...
  tmp = strchr("0123456789.", (int )c___0);
  return ((unsigned long )tmp != (unsigned long )((char *)0));
}
}
int isDigit(char const c___0 )
...
#pragma merger("0","./tot_info.i","-g,-g")
...
double LGamma(double LGamma_x ) ;
double QGamma(char global_line[256] , long global_f[1000] , int global_r , int global_c ,
              double global_cof[6] , double a , double x ) ;
double QChiSq(double QChiSq_chisq , int QChiSq_df ) ;
...
int main1(int argc , char **argv )
{
  ...
  while (1) {
    ...
    while (i___0 < r) {
      j = 0;
      while (j < c) {
        tmp___1 = scanf((char const * __restrict )" %ld", & f[i___0 * c + j]);
        if (tmp___1 != 1) {
...
double LGamma(double LGamma_x )
{
  ...
  int LGamma_j ;
  double LGamma_pix ;
  double LGamma_tmp___0 ;
  double LGamma_tmp___1 ;
  double LGamma_tmp___2 ;
  double LGamma_tmp___3 ;
  int LGamma___cil_tmp10 ;
  char *symName ;
  void *addr ;
  char in[15] ;
  ...
  {
  LGamma_x -= (double )1;
  handleAssignmentSymbolically("LGamma_x", "(- LGamma_x 1)", & LGamma_x, & LGamma_x,
                               2);
  if (LGamma_x < 0.0) {
    LGamma_pix = 3.14159265358979323846264338327950288419716939937511 * LGamma_x;
    handleAssignmentSymbolically("LGamma_pix", "(* 3.14159265358979323846264338327950288419716939937511 LGamma_x)",
                                 & LGamma_pix, & LGamma_pix, 2);
    LGamma_tmp___0 = sin(LGamma_pix);
    add_entryToSTable("LGamma_tmp___0", "Function", & LGamma_tmp___0, & LGamma_tmp___0,
                      2);
    LGamma_tmp___1 = log(LGamma_pix / LGamma_tmp___0);
    add_entryToSTable("LGamma_tmp___1", "Function", & LGamma_tmp___1, & LGamma_tmp___1,
                      2);
    {
    mapConcolicValues("LGamma_tmp___1", & LGamma_tmp___1);
    return (LGamma_tmp___1);
    }
  }
  LGamma_tmp = LGamma_x + 5.5;
  handleAssignmentSymbolically("LGamma_tmp", "(+ LGamma_x 5.5)", & LGamma_tmp, & LGamma_tmp,
                               2);
  LGamma_tmp___2 = log(LGamma_tmp);
  add_entryToSTable("LGamma_tmp___2", "Function", & LGamma_tmp___2, & LGamma_tmp___2,
                    2);
  LGamma_tmp -= (LGamma_x + 0.5) * LGamma_tmp___2;
  handleAssignmentSymbolically("LGamma_tmp", "(- LGamma_tmp (* (+ LGamma_x 0.5) LGamma_tmp___2))",
                               & LGamma_tmp, & LGamma_tmp, 2);
  LGamma_ser = 1.0;
  add_entryToSTable("LGamma_ser", "Constant", & LGamma_ser, & LGamma_ser, 2);
  LGamma_j = 0;
  add_entryToSTable("LGamma_j", "Constant", & LGamma_j, & LGamma_j, 1);
  {
  if (LGamma_j < 6) {
    LGamma_x += (double )1;
    handleAssignmentSymbolically("LGamma_x", "(+ LGamma_x 1)", & LGamma_x, & LGamma_x,
                                 2);
    LGamma_ser += cof[LGamma_j] / LGamma_x;
    addEntryToVariableTable("cof[LGamma_j]", LGamma_j);
    add_entryToArraySTable("cof", LGamma_j, "cof20", & cof[LGamma_j], & cof[LGamma_j],
                           1);
    handleAssignmentSymbolically("LGamma_ser", "(+ LGamma_ser (/ cof[LGamma_j] LGamma_x))",
                                 & LGamma_ser, & LGamma_ser, 2);
    LGamma_j ++;
    handleAssignmentSymbolically("LGamma_j", "(+ LGamma_j 1)", & LGamma_j, & LGamma_j,
                                 1);
  }
  if (LGamma_j < 6) {
    LGamma_x += (double )1;
    ...
    LGamma_ser += cof[LGamma_j] / LGamma_x;
    addEntryToVariableTable("cof[LGamma_j]", LGamma_j);
    add_entryToArraySTable("cof", LGamma_j, "cof21", & cof[LGamma_j], & cof[LGamma_j],
                           1);
    handleAssignmentSymbolically("LGamma_ser", "(+ LGamma_ser (/ cof[LGamma_j] LGamma_x))",
                                 & LGamma_ser, & LGamma_ser, 2);
    LGamma_j ++;
  ...
  LGamma_tmp___3 = log(2.50662827465 * LGamma_ser);
  add_entryToSTable("LGamma_tmp___3", "Function", & LGamma_tmp___3, & LGamma_tmp___3,
                    2);
  {
  LGamma___cil_tmp10 = - LGamma_tmp + LGamma_tmp___3;
  handleAssignmentSymbolically("LGamma___cil_tmp10", "(+ (- LGamma_tmp) LGamma_tmp___3)",
                               & LGamma___cil_tmp10, & LGamma___cil_tmp10, 1);
  {
  mapConcolicValues("LGamma___cil_tmp10", & LGamma___cil_tmp10);
  return (LGamma___cil_tmp10);
  }
  }
}
}
static double gser(double gser_a , double gser_x )
{
  double gser_ap ;
  double gser_del ;
  double gser_sum ;
  ...
  double gser_tmp___2 ;
  double gser_tmp___3 ;
  int gser___cil_tmp12 ;
  int gser___cil_tmp13 ;
  int gser___cil_tmp14 ;
  char *symName ;
  void *addr ;
  char in[15] ;
  ...
  if (gser_x <= 0.0) {
    {
    gser___cil_tmp12 = 0.0;
    add_entryToSTable("gser___cil_tmp12", "Constant", & gser___cil_tmp12, & gser___cil_tmp12,
                      1);
    {
    mapConcolicValues("gser___cil_tmp12", & gser___cil_tmp12);
    return (gser___cil_tmp12);
    }
    }
  }
  gser_ap = gser_a;
  handleAssignmentSymbolically("gser_ap", "gser_a", & gser_a, & gser_a, 2);
  gser_sum = 1.0 / gser_ap;
  handleAssignmentSymbolically("gser_sum", "(/ 1.0 gser_ap)", & gser_sum, & gser_sum,
                               2);
  gser_del = gser_sum;
  handleAssignmentSymbolically("gser_del", "gser_sum", & gser_sum, & gser_sum, 2);
  gser_n = 1;
  add_entryToSTable("gser_n", "Constant", & gser_n, & gser_n, 1);
  {
  if (gser_n <= 100) {
    gser_ap += (double )1;
    handleAssignmentSymbolically("gser_ap", "(+ gser_ap 1)", & gser_ap, & gser_ap,
                                 2);
    gser_del *= gser_x / gser_ap;
    handleAssignmentSymbolically("gser_del", "(* gser_del (/ gser_x gser_ap))", & gser_del,
                                 & gser_del, 2);
    gser_sum += gser_del;
    handleAssignmentSymbolically("gser_sum", "(+ gser_sum gser_del)", & gser_sum,
                                 & gser_sum, 2);
    if (gser_del < (double )0) {
      gser_tmp___2 = - gser_del;
      handleAssignmentSymbolically("gser_tmp___2", "(- gser_del)", & gser_tmp___2,
                                   & gser_tmp___2, 2);
    } else {
      gser_tmp___2 = gser_del;
      handleAssignmentSymbolically("gser_tmp___2", "gser_del", & gser_del, & gser_del,
                                   2);
    }
    if (gser_sum < (double )0) {
      gser_tmp___3 = - gser_sum;
      handleAssignmentSymbolically("gser_tmp___3", "(- gser_sum)", & gser_tmp___3,
                                   & gser_tmp___3, 2);
    } else {
      gser_tmp___3 = gser_sum;
      handleAssignmentSymbolically("gser_tmp___3", "gser_sum", & gser_sum, & gser_sum,
                                   2);
    }
    if (gser_tmp___2 < gser_tmp___3 * 3.0e-7) {
      gser_tmp = log(gser_x);
      add_entryToSTable("gser_tmp", "Function", & gser_tmp, & gser_tmp, 2);
      funcEntry("(double,LGamma_x,variable,gser_a)", "LGamma_tmp LGamma_ser LGamma_j LGamma_pix LGamma_tmp___0 LGamma_tmp___1 LGamma_tmp___2 LGamma_tmp___3 LGamma___cil_tmp10",
                "LGamma");
      gser_tmp___0 = LGamma(gser_a);
      funcExit();
      add_entryToSTable("gser_tmp___0", ret_SymValue, ret_ConValue, & gser_tmp___0,
                        2);
      gser_tmp___1 = exp((- gser_x + gser_a * gser_tmp) - gser_tmp___0);
      add_entryToSTable("gser_tmp___1", "Function", & gser_tmp___1, & gser_tmp___1,
                        2);
      {
      gser___cil_tmp13 = gser_sum * gser_tmp___1;
      handleAssignmentSymbolically("gser___cil_tmp13", "(* gser_sum gser_tmp___1)",
                                   & gser___cil_tmp13, & gser___cil_tmp13, 1);
      {
      mapConcolicValues("gser___cil_tmp13", & gser___cil_tmp13);
      return (gser___cil_tmp13);
      }
      }
    }
    gser_n ++;
    handleAssignmentSymbolically("gser_n", "(+ gser_n 1)", & gser_n, & gser_n, 1);
  }
  if (gser_n <= 100) {
    gser_ap += (double )1;
    handleAssignmentSymbolically("gser_ap", "(+ gser_ap 1)", & gser_ap, & gser_ap,
  ...
  {
  gser___cil_tmp14 = (double )0;
  add_entryToSTable("gser___cil_tmp14", "Constant", & gser___cil_tmp14, & gser___cil_tmp14,
                    1);
  {
  mapConcolicValues("gser___cil_tmp14", & gser___cil_tmp14);
  return (gser___cil_tmp14);
  }
  }
}
}
static double gcf(double gcf_a , double gcf_x )
{
  int gcf_n ;
  double gcf_gold ;
  double gcf_fac ;
  ...
  {
  gcf_gold = 0.0;
  add_entryToSTable("gcf_gold", "Constant", & gcf_gold, & gcf_gold, 2);
  gcf_fac = 1.0;
  add_entryToSTable("gcf_fac", "Constant", & gcf_fac, & gcf_fac, 2);
  gcf_b1 = 1.0;
  add_entryToSTable("gcf_b1", "Constant", & gcf_b1, & gcf_b1, 2);
  gcf_b0 = 0.0;
  add_entryToSTable("gcf_b0", "Constant", & gcf_b0, & gcf_b0, 2);
  gcf_a0 = 1.0;
  add_entryToSTable("gcf_a0", "Constant", & gcf_a0, & gcf_a0, 2);
  gcf_a1 = gcf_x;
  handleAssignmentSymbolically("gcf_a1", "gcf_x", & gcf_x, & gcf_x, 2);
  gcf_n = 0;
  add_entryToSTable("gcf_n", "Constant", & gcf_n, & gcf_n, 1);
  {
  if (gcf_n <= 100) {
    gcf_an = (double )gcf_n;
    handleAssignmentSymbolically("gcf_an", "gcf_n", & gcf_an, & gcf_an, 2);
    gcf_ana = gcf_an - gcf_a;
    handleAssignmentSymbolically("gcf_ana", "(- gcf_an gcf_a)", & gcf_ana, & gcf_ana,
                                 2);
    gcf_a0 = (gcf_a1 + gcf_a0 * gcf_ana) * gcf_fac;
    handleAssignmentSymbolically("gcf_a0", "(* (+ gcf_a1 (* gcf_a0 gcf_ana)) gcf_fac)",
                                 & gcf_a0, & gcf_a0, 2);
    gcf_b0 = (gcf_b1 + gcf_b0 * gcf_ana) * gcf_fac;
    handleAssignmentSymbolically("gcf_b0", "(* (+ gcf_b1 (* gcf_b0 gcf_ana)) gcf_fac)",
                                 & gcf_b0, & gcf_b0, 2);
    gcf_anf = gcf_an * gcf_fac;
    handleAssignmentSymbolically("gcf_anf", "(* gcf_an gcf_fac)", & gcf_anf, & gcf_anf,
                                 2);
    gcf_b1 = gcf_x * gcf_b0 + gcf_anf * gcf_b1;
    handleAssignmentSymbolically("gcf_b1", "(+ (* gcf_x gcf_b0) (* gcf_anf gcf_b1))",
                                 & gcf_b1, & gcf_b1, 2);
    gcf_a1 = gcf_x * gcf_a0 + gcf_anf * gcf_a1;
    handleAssignmentSymbolically("gcf_a1", "(+ (* gcf_x gcf_a0) (* gcf_anf gcf_a1))",
                                 & gcf_a1, & gcf_a1, 2);
    if (gcf_a1 != 0.0) {
      gcf_fac = 1.0 / gcf_a1;
      handleAssignmentSymbolically("gcf_fac", "(/ 1.0 gcf_a1)", & gcf_fac, & gcf_fac,
                                   2);
      gcf_g = gcf_b1 * gcf_fac;
      handleAssignmentSymbolically("gcf_g", "(* gcf_b1 gcf_fac)", & gcf_g, & gcf_g,
                                   2);
      gcf_gold = gcf_g - gcf_gold;
      handleAssignmentSymbolically("gcf_gold", "(- gcf_g gcf_gold)", & gcf_gold, & gcf_gold,
                                   2);
      if (gcf_gold < (double )0) {
        gcf_tmp___2 = - gcf_gold;
        handleAssignmentSymbolically("gcf_tmp___2", "(- gcf_gold)", & gcf_tmp___2,
                                     & gcf_tmp___2, 2);
      } else {
        gcf_tmp___2 = gcf_gold;
        handleAssignmentSymbolically("gcf_tmp___2", "gcf_gold", & gcf_gold, & gcf_gold,
                                     2);
      }
      if (gcf_g < (double )0) {
        gcf_tmp___3 = - gcf_g;
        handleAssignmentSymbolically("gcf_tmp___3", "(- gcf_g)", & gcf_tmp___3, & gcf_tmp___3,
                                     2);
      } else {
        gcf_tmp___3 = gcf_g;
        handleAssignmentSymbolically("gcf_tmp___3", "gcf_g", & gcf_g, & gcf_g, 2);
      }
      if (gcf_tmp___2 < 3.0e-7 * gcf_tmp___3) {
        gcf_tmp = log(gcf_x);
        add_entryToSTable("gcf_tmp", "Function", & gcf_tmp, & gcf_tmp, 2);
        funcEntry("(double,LGamma_x,variable,gcf_a)", "LGamma_tmp LGamma_ser LGamma_j LGamma_pix LGamma_tmp___0 LGamma_tmp___1 LGamma_tmp___2 LGamma_tmp___3 LGamma___cil_tmp10",
                  "LGamma");
        gcf_tmp___0 = LGamma(gcf_a);
        funcExit();
        add_entryToSTable("gcf_tmp___0", ret_SymValue, ret_ConValue, & gcf_tmp___0,
                          2);
        gcf_tmp___1 = exp((- gcf_x + gcf_a * gcf_tmp) - gcf_tmp___0);
        add_entryToSTable("gcf_tmp___1", "Function", & gcf_tmp___1, & gcf_tmp___1,
                          2);
        {
        gcf___cil_tmp19 = gcf_tmp___1 * gcf_g;
        handleAssignmentSymbolically("gcf___cil_tmp19", "(* gcf_tmp___1 gcf_g)", & gcf___cil_tmp19,
                                     & gcf___cil_tmp19, 1);
        {
        mapConcolicValues("gcf___cil_tmp19", & gcf___cil_tmp19);
        return (gcf___cil_tmp19);
        }
        }
      }
      gcf_gold = gcf_g;
      handleAssignmentSymbolically("gcf_gold", "gcf_g", & gcf_g, & gcf_g, 2);
    }
    gcf_n ++;
    handleAssignmentSymbolically("gcf_n", "(+ gcf_n 1)", & gcf_n, & gcf_n, 1);
  }
  if (gcf_n <= 100) {
    gcf_an = (double )gcf_n;
    handleAssignmentSymbolically("gcf_an", "gcf_n", & gcf_an, & gcf_an, 2);
  ...
  {
  gcf___cil_tmp20 = (double )0;
  add_entryToSTable("gcf___cil_tmp20", "Constant", & gcf___cil_tmp20, & gcf___cil_tmp20,
                    1);
  {
  mapConcolicValues("gcf___cil_tmp20", & gcf___cil_tmp20);
  return (gcf___cil_tmp20);
  }
  }
}
}
...
void createSidTable(void)
{
  ...
  {
  add_condition(1, "(< x (+ a 1.0))", "(not (< x (+ a 1.0)))", 0, 0);
}
}
struct arguments {
   char line[256] ;
              ...
              double global_cof[6] , double a , double x )
{
  ...
  sprintf(__cil_tmp9, "\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%f\t%f\t%f\t%f\t%f\t%f\t%f\n",
          line[0], line[1], line[2], line[3], line[4], f[0], f[1], f[2], f[3], f[4],
          r, c, cof[0], cof[1], cof[2], cof[3], cof[4], a, x);
  printTestCase("tot_info_QGamma_1435616290.tc", __cil_tmp9);
  add_entryToSTable("x", "s6", & x, & x, 2);
  add_entryToSTable("a", "s5", & a, & a, 2);
  add_entryToSTable("c", "s3", & c, & c, 1);
  add_entryToSTable("r", "s2", & r, & r, 1);
  {
  exp_outcome = x < a + 1.0;
  handleAssignmentSymbolically("exp_outcome", "(< x (+ a 1.0))", & exp_outcome, & exp_outcome,
                               1);
  overall_outcome = (int )getConditionalOutcome(1, exp_outcome);
  if (overall_outcome) {
    setBranchInfo(1, 1, 0);
    setTrueExpr(1, "(< x (+ a 1.0))");
    setFalseExpr(1, "(not (< x (+ a 1.0)))");
    addToTree(1, 1, "(< x (+ a 1.0))", "(not (< x (+ a 1.0)))", 0, 1);
    delete_allVariableTableEntry();
    funcEntry("(double,gser_a,variable,a)#(double,gser_x,variable,x)", "gser_ap gser_del gser_sum gser_n gser_tmp gser_tmp___0 gser_tmp___1 gser_tmp___2 gser_tmp___3 gser___cil_tmp12 gser___cil_tmp13 gser___cil_tmp14",
              "gser");
    tmp = gser(a, x);
    funcExit();
    add_entryToSTable("tmp", ret_SymValue, ret_ConValue, & tmp, 2);
    tmp___1 = 1.0 - tmp;
    handleAssignmentSymbolically("tmp___1", "(- 1.0 tmp)", & tmp___1, & tmp___1, 2);
  } else {
    setBranchInfo(1, 0, 1);
    setTrueExpr(1, "(< x (+ a 1.0))");
    setFalseExpr(1, "(not (< x (+ a 1.0)))");
    addToTree(1, 1, "(< x (+ a 1.0))", "(not (< x (+ a 1.0)))", 0, 0);
    delete_allVariableTableEntry();
    funcEntry("(double,gcf_a,variable,a)#(double,gcf_x,variable,x)", "gcf_n gcf_gold gcf_fac gcf_b1 gcf_b0 gcf_a0 gcf_a1 gcf_anf gcf_an gcf_ana gcf_g gcf_tmp gcf_tmp___0 gcf_tmp___1 gcf_tmp___2 gcf_tmp___3 gcf___cil_tmp19 gcf___cil_tmp20",
              "gcf");
    tmp___0 = gcf(a, x);
    funcExit();
    add_entryToSTable("tmp___0", ret_SymValue, ret_ConValue, & tmp___0, 2);
    tmp___1 = tmp___0;
    handleAssignmentSymbolically("tmp___1", "tmp___0", & tmp___0, & tmp___0, 2);
  }
  }
...
double QChiSq(double QChiSq_chisq , int QChiSq_df )
{
  ...
  {
  mapConcolicValues("QChiSq_tmp", & QChiSq_tmp);
  return (QChiSq_tmp);
  }
}
}
...
void callInstrumentedFun(void)
{
  ...
  {
  enQueue();
  QGamma(argvar.line, argvar.f, argvar.r, argvar.c, argvar.cof, argvar.a, argvar.x);
}
}
void main(void)
{
  ...
  while (1) {
    if (__cil_tmp11 >= 256) {
      ...
      __cil_tmp10 = rand();
      argvar.line[__cil_tmp11] = __cil_tmp10 % 20;
      __cil_tmp11 ++;
    }
  }
  __cil_tmp9 = 0;
  while (1) {
    if (__cil_tmp9 >= 1000) {
      ...
      __cil_tmp8 = rand();
      argvar.f[__cil_tmp9] = __cil_tmp8 % 20;
      __cil_tmp9 ++;
    }
  }
  __cil_tmp7 = rand();
  argvar.r = __cil_tmp7 % 20;
  __cil_tmp6 = rand();
  argvar.c = __cil_tmp6 % 20;
  __cil_tmp5 = 0;
  while (1) {
    if (__cil_tmp5 >= 6) {
      break;
      ...
      __cil_tmp4 = rand();
      argvar.cof[__cil_tmp5] = __cil_tmp4 % 20;
      __cil_tmp5 ++;
    }
  }
  __cil_tmp3 = rand();
