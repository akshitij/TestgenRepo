...
{ 
  ...
  {
  add_condition(2, "(= whatever_a -1)", "(not (= whatever_a -1))", 0, 0);
}
}
struct arguments {
   int whatever_a ;
...
{ 
  ...
  add_entryToSTable("whatever_a", "s0", & whatever_a, & whatever_a, 1);
  foo();
  {
  exp_outcome = whatever_a == -1;
  handleAssignmentSymbolically("exp_outcome", "(= whatever_a -1)", & exp_outcome,
                               & exp_outcome, 1);
  overall_outcome = (int )getConditionalOutcome(2, exp_outcome);
  if (overall_outcome) {
    setBranchInfo(2, 1, 0);
    setTrueExpr(2, "(= whatever_a -1)");
    setFalseExpr(2, "(not (= whatever_a -1))");
    addToTree(2, 1, "(= whatever_a -1)", "(not (= whatever_a -1))", 0, 1);
    delete_allVariableTableEntry();
    b = 1;
    ...
    setTrueExpr(2, "(= whatever_a -1)");
    setFalseExpr(2, "(not (= whatever_a -1))");
    addToTree(2, 1, "(= whatever_a -1)", "(not (= whatever_a -1))", 0, 0);
    delete_allVariableTableEntry();
    b = 2;
    add_entryToSTable("b", "Constant", & b, & b, 1);
  }
...
{ 
  ...
  {
  __cil_tmp2 = rand();
  argvar.whatever_a = __cil_tmp2 % 20;
  initSID();
  isCopyOfHolder();
  createCDG();
  createSidTable();
