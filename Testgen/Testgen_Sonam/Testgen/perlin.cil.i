...
void deleteArrayTable(void)
{
  ...
  while (p___0) {
    while (1) {
      if ((unsigned long )p___0->hh.prev == (unsigned long )((void *)0)) {
        if ((unsigned long )p___0->hh.next == (unsigned long )((void *)0)) {
        ...
        _L:
        _hd_hh_del = & p___0->hh;
        if ((unsigned long )p___0 == (unsigned long )((void *)((char *)(arraySTable->hh.tbl)->tail - (arraySTable->hh.tbl)->hho))) {
          (arraySTable->hh.tbl)->tail = (UT_hash_handle *)((ptrdiff_t )p___0->hh.prev + (arraySTable->hh.tbl)->hho);
        }
        if (p___0->hh.prev) {
          ((UT_hash_handle *)((ptrdiff_t )p___0->hh.prev + (arraySTable->hh.tbl)->hho))->next = p___0->hh.next;
        } else {
          while (1) {
            arraySTable = (struct arraySym_table *)p___0->hh.next;
            break;
          }
        }
        if (_hd_hh_del->next) {
...
static int permutation[256] =
  { 151, 160, 137, 91,
        ...
        128, 195, 78, 66,
        215, 61, 156, 180};
static double fade(double fade_t )
{
  int fade___cil_tmp2 ;
  char *symName ;
  void *addr ;
  ...
  {
  {
  fade___cil_tmp2 = ((fade_t * fade_t) * fade_t) * (fade_t * (fade_t * (double )6 - (double )15) + (double )10);
  handleAssignmentSymbolically("fade___cil_tmp2", "(* (* (* fade_t fade_t) fade_t) (+ (* fade_t (- (* fade_t 6) 15)) 10))",
                               & fade___cil_tmp2, & fade___cil_tmp2, 1);
  {
  mapConcolicValues("fade___cil_tmp2", & fade___cil_tmp2);
  return (fade___cil_tmp2);
  }
  }
}
}
static double lerp(double lerp_t , double lerp_a , double lerp_b )
{
  int lerp___cil_tmp4 ;
  char *symName ;
  void *addr ;
  ...
  {
  {
  lerp___cil_tmp4 = lerp_a + lerp_t * (lerp_b - lerp_a);
  handleAssignmentSymbolically("lerp___cil_tmp4", "(+ lerp_a (* lerp_t (- lerp_b lerp_a)))",
                               & lerp___cil_tmp4, & lerp___cil_tmp4, 1);
  {
  mapConcolicValues("lerp___cil_tmp4", & lerp___cil_tmp4);
  return (lerp___cil_tmp4);
  }
  }
}
}
static double grad(int grad_hash , double grad_x , double grad_y , double grad_z )
{
  int grad_h ;
  double grad_u ;
  double grad_tmp ;
  ...
  {
  grad_h = grad_hash & 15;
  handleAssignmentSymbolically("grad_h", "(& grad_hash 15)", & grad_h, & grad_h, 1);
  if (grad_h < 8) {
    grad_tmp = grad_x;
    handleAssignmentSymbolically("grad_tmp", "grad_x", & grad_x, & grad_x, 2);
  } else {
    grad_tmp = grad_y;
    handleAssignmentSymbolically("grad_tmp", "grad_y", & grad_y, & grad_y, 2);
  }
  grad_u = grad_tmp;
  handleAssignmentSymbolically("grad_u", "grad_tmp", & grad_tmp, & grad_tmp, 2);
  if (grad_h < 4) {
    grad_tmp___1 = grad_y;
    handleAssignmentSymbolically("grad_tmp___1", "grad_y", & grad_y, & grad_y, 2);
  } else {
    if (grad_h == 12) {
      grad_tmp___0 = grad_x;
      handleAssignmentSymbolically("grad_tmp___0", "grad_x", & grad_x, & grad_x, 2);
    } else
    if (grad_h == 14) {
      grad_tmp___0 = grad_x;
      handleAssignmentSymbolically("grad_tmp___0", "grad_x", & grad_x, & grad_x, 2);
      ...
      grad_tmp___0 = grad_z;
      handleAssignmentSymbolically("grad_tmp___0", "grad_z", & grad_z, & grad_z, 2);
    }
    grad_tmp___1 = grad_tmp___0;
    handleAssignmentSymbolically("grad_tmp___1", "grad_tmp___0", & grad_tmp___0, & grad_tmp___0,
                                 2);
  }
  grad_v = grad_tmp___1;
  handleAssignmentSymbolically("grad_v", "grad_tmp___1", & grad_tmp___1, & grad_tmp___1,
                               2);
  if ((grad_h & 1) == 0) {
    grad_tmp___2 = grad_u;
    handleAssignmentSymbolically("grad_tmp___2", "grad_u", & grad_u, & grad_u, 2);
  } else {
    grad_tmp___2 = - grad_u;
    handleAssignmentSymbolically("grad_tmp___2", "(- grad_u)", & grad_tmp___2, & grad_tmp___2,
                                 2);
  }
  if ((grad_h & 2) == 0) {
    grad_tmp___3 = grad_v;
    handleAssignmentSymbolically("grad_tmp___3", "grad_v", & grad_v, & grad_v, 2);
  } else {
    grad_tmp___3 = - grad_v;
    handleAssignmentSymbolically("grad_tmp___3", "(- grad_v)", & grad_tmp___3, & grad_tmp___3,
                                 2);
  }
  {
  grad___cil_tmp13 = grad_tmp___2 + grad_tmp___3;
  handleAssignmentSymbolically("grad___cil_tmp13", "(+ grad_tmp___2 grad_tmp___3)",
                               & grad___cil_tmp13, & grad___cil_tmp13, 1);
  {
  mapConcolicValues("grad___cil_tmp13", & grad___cil_tmp13);
  return (grad___cil_tmp13);
  }
  }
}
}
...
static double noise(int global_p[512] , int global_permutation[256] , double x , double y ,
                    double z )
{
  int X ;
  double tmp ;
  ...
  {
  __cil_tmp63 = malloc(100 * sizeof(char ));
  add_entryToSTable("__cil_tmp63", "Function", & __cil_tmp63, & __cil_tmp63, -1);
  ...
  tmp = floor(x);
  add_entryToSTable("tmp", "Function", & tmp, & tmp, 2);
  X = (int )tmp & 255;
  handleAssignmentSymbolically("X", "(& tmp 255)", & X, & X, 1);
  tmp___0 = floor(y);
  add_entryToSTable("tmp___0", "Function", & tmp___0, & tmp___0, 2);
  Y = (int )tmp___0 & 255;
  handleAssignmentSymbolically("Y", "(& tmp___0 255)", & Y, & Y, 1);
  tmp___1 = floor(z);
  add_entryToSTable("tmp___1", "Function", & tmp___1, & tmp___1, 2);
  Z = (int )tmp___1 & 255;
  handleAssignmentSymbolically("Z", "(& tmp___1 255)", & Z, & Z, 1);
  tmp___2 = floor(x);
  add_entryToSTable("tmp___2", "Function", & tmp___2, & tmp___2, 2);
  x -= tmp___2;
  handleAssignmentSymbolically("x", "(- x tmp___2)", & x, & x, 2);
  tmp___3 = floor(y);
  add_entryToSTable("tmp___3", "Function", & tmp___3, & tmp___3, 2);
  y -= tmp___3;
  handleAssignmentSymbolically("y", "(- y tmp___3)", & y, & y, 2);
  tmp___4 = floor(z);
  add_entryToSTable("tmp___4", "Function", & tmp___4, & tmp___4, 2);
  z -= tmp___4;
  handleAssignmentSymbolically("z", "(- z tmp___4)", & z, & z, 2);
  funcEntry("(double,fade_t,variable,x)", "fade___cil_tmp2", "fade");
  tmp___5 = fade(x);
  funcExit();
  add_entryToSTable("tmp___5", ret_SymValue, ret_ConValue, & tmp___5, 2);
  u = tmp___5;
  handleAssignmentSymbolically("u", "tmp___5", & tmp___5, & tmp___5, 2);
  funcEntry("(double,fade_t,variable,y)", "fade___cil_tmp2", "fade");
  tmp___6 = fade(y);
  funcExit();
  add_entryToSTable("tmp___6", ret_SymValue, ret_ConValue, & tmp___6, 2);
  v = tmp___6;
  handleAssignmentSymbolically("v", "tmp___6", & tmp___6, & tmp___6, 2);
  funcEntry("(double,fade_t,variable,z)", "fade___cil_tmp2", "fade");
  tmp___7 = fade(z);
  funcExit();
  add_entryToSTable("tmp___7", ret_SymValue, ret_ConValue, & tmp___7, 2);
  w = tmp___7;
  handleAssignmentSymbolically("w", "tmp___7", & tmp___7, & tmp___7, 2);
  A = p[X] + Y;
  addEntryToVariableTable("p[X]", X);
  add_entryToArraySTable("p", X, "p20", & p[X], & p[X], 1);
  handleAssignmentSymbolically("A", "(+ p[X] Y)", & A, & A, 1);
  AA = p[A] + Z;
  addEntryToVariableTable("p[A]", A);
  add_entryToArraySTable("p", A, "p21", & p[A], & p[A], 1);
  handleAssignmentSymbolically("AA", "(+ p[A] Z)", & AA, & AA, 1);
  AB = p[A + 1] + Z;
  addEntryToVariableTable("p[A + 1]", A + 1);
  add_entryToArraySTable("p", A + 1, "p22", & p[A + 1], & p[A + 1], 1);
  handleAssignmentSymbolically("AB", "(+ p[A + 1] Z)", & AB, & AB, 1);
  B = p[X + 1] + Y;
  addEntryToVariableTable("p[X + 1]", X + 1);
  add_entryToArraySTable("p", X + 1, "p23", & p[X + 1], & p[X + 1], 1);
  handleAssignmentSymbolically("B", "(+ p[X + 1] Y)", & B, & B, 1);
  BA = p[B] + Z;
  addEntryToVariableTable("p[B]", B);
  add_entryToArraySTable("p", B, "p24", & p[B], & p[B], 1);
  handleAssignmentSymbolically("BA", "(+ p[B] Z)", & BA, & BA, 1);
  BB = p[B + 1] + Z;
  addEntryToVariableTable("p[B + 1]", B + 1);
  add_entryToArraySTable("p", B + 1, "p25", & p[B + 1], & p[B + 1], 1);
  handleAssignmentSymbolically("BB", "(+ p[B + 1] Z)", & BB, & BB, 1);
  __cil_tmp40 = p[BB + 1];
  addEntryToVariableTable("p[BB + 1]", BB + 1);
  add_entryToArraySTable("p", BB + 1, "p26", & p[BB + 1], & p[BB + 1], 1);
  handleAssignmentSymbolically("__cil_tmp40", "p[BB + 1]", & p[BB + 1], & p[BB + 1],
                               1);
  __cil_tmp41 = x - (double )1;
  handleAssignmentSymbolically("__cil_tmp41", "(- x 1)", & __cil_tmp41, & __cil_tmp41,
                               2);
  __cil_tmp42 = y - (double )1;
  handleAssignmentSymbolically("__cil_tmp42", "(- y 1)", & __cil_tmp42, & __cil_tmp42,
                               2);
  __cil_tmp43 = z - (double )1;
  handleAssignmentSymbolically("__cil_tmp43", "(- z 1)", & __cil_tmp43, & __cil_tmp43,
                               2);
  funcEntry("(int,grad_hash,variable,__cil_tmp40)#(double,grad_x,variable,__cil_tmp41)#(double,grad_y,variable,__cil_tmp42)#(double,grad_z,variable,__cil_tmp43)",
            "grad_h grad_u grad_tmp grad_v grad_tmp___0 grad_tmp___1 grad_tmp___2 grad_tmp___3 grad___cil_tmp13",
            "grad");
  tmp___8 = grad(__cil_tmp40, __cil_tmp41, __cil_tmp42, __cil_tmp43);
  funcExit();
  add_entryToSTable("tmp___8", ret_SymValue, ret_ConValue, & tmp___8, 2);
  __cil_tmp44 = p[AB + 1];
  addEntryToVariableTable("p[AB + 1]", AB + 1);
  add_entryToArraySTable("p", AB + 1, "p27", & p[AB + 1], & p[AB + 1], 1);
  handleAssignmentSymbolically("__cil_tmp44", "p[AB + 1]", & p[AB + 1], & p[AB + 1],
                               1);
  __cil_tmp45 = y - (double )1;
  handleAssignmentSymbolically("__cil_tmp45", "(- y 1)", & __cil_tmp45, & __cil_tmp45,
                               2);
  __cil_tmp46 = z - (double )1;
  handleAssignmentSymbolically("__cil_tmp46", "(- z 1)", & __cil_tmp46, & __cil_tmp46,
                               2);
  funcEntry("(int,grad_hash,variable,__cil_tmp44)#(double,grad_x,variable,x)#(double,grad_y,variable,__cil_tmp45)#(double,grad_z,variable,__cil_tmp46)",
            "grad_h grad_u grad_tmp grad_v grad_tmp___0 grad_tmp___1 grad_tmp___2 grad_tmp___3 grad___cil_tmp13",
            "grad");
  tmp___9 = grad(__cil_tmp44, x, __cil_tmp45, __cil_tmp46);
  funcExit();
  add_entryToSTable("tmp___9", ret_SymValue, ret_ConValue, & tmp___9, 2);
  funcEntry("(double,lerp_t,variable,u)#(double,lerp_a,variable,tmp___9)#(double,lerp_b,variable,tmp___8)",
            "lerp___cil_tmp4", "lerp");
  tmp___10 = lerp(u, tmp___9, tmp___8);
  funcExit();
  add_entryToSTable("tmp___10", ret_SymValue, ret_ConValue, & tmp___10, 2);
  __cil_tmp47 = p[BA + 1];
  addEntryToVariableTable("p[BA + 1]", BA + 1);
  add_entryToArraySTable("p", BA + 1, "p28", & p[BA + 1], & p[BA + 1], 1);
  handleAssignmentSymbolically("__cil_tmp47", "p[BA + 1]", & p[BA + 1], & p[BA + 1],
                               1);
  __cil_tmp48 = x - (double )1;
  handleAssignmentSymbolically("__cil_tmp48", "(- x 1)", & __cil_tmp48, & __cil_tmp48,
                               2);
  __cil_tmp49 = z - (double )1;
  handleAssignmentSymbolically("__cil_tmp49", "(- z 1)", & __cil_tmp49, & __cil_tmp49,
                               2);
  funcEntry("(int,grad_hash,variable,__cil_tmp47)#(double,grad_x,variable,__cil_tmp48)#(double,grad_y,variable,y)#(double,grad_z,variable,__cil_tmp49)",
            "grad_h grad_u grad_tmp grad_v grad_tmp___0 grad_tmp___1 grad_tmp___2 grad_tmp___3 grad___cil_tmp13",
            "grad");
  tmp___11 = grad(__cil_tmp47, __cil_tmp48, y, __cil_tmp49);
  funcExit();
  add_entryToSTable("tmp___11", ret_SymValue, ret_ConValue, & tmp___11, 2);
  __cil_tmp50 = p[AA + 1];
  addEntryToVariableTable("p[AA + 1]", AA + 1);
  add_entryToArraySTable("p", AA + 1, "p29", & p[AA + 1], & p[AA + 1], 1);
  handleAssignmentSymbolically("__cil_tmp50", "p[AA + 1]", & p[AA + 1], & p[AA + 1],
                               1);
  __cil_tmp51 = z - (double )1;
  handleAssignmentSymbolically("__cil_tmp51", "(- z 1)", & __cil_tmp51, & __cil_tmp51,
                               2);
  funcEntry("(int,grad_hash,variable,__cil_tmp50)#(double,grad_x,variable,x)#(double,grad_y,variable,y)#(double,grad_z,variable,__cil_tmp51)",
            "grad_h grad_u grad_tmp grad_v grad_tmp___0 grad_tmp___1 grad_tmp___2 grad_tmp___3 grad___cil_tmp13",
            "grad");
  tmp___12 = grad(__cil_tmp50, x, y, __cil_tmp51);
  funcExit();
  add_entryToSTable("tmp___12", ret_SymValue, ret_ConValue, & tmp___12, 2);
  funcEntry("(double,lerp_t,variable,u)#(double,lerp_a,variable,tmp___12)#(double,lerp_b,variable,tmp___11)",
            "lerp___cil_tmp4", "lerp");
  tmp___13 = lerp(u, tmp___12, tmp___11);
  funcExit();
  add_entryToSTable("tmp___13", ret_SymValue, ret_ConValue, & tmp___13, 2);
  funcEntry("(double,lerp_t,variable,v)#(double,lerp_a,variable,tmp___13)#(double,lerp_b,variable,tmp___10)",
            "lerp___cil_tmp4", "lerp");
  tmp___14 = lerp(v, tmp___13, tmp___10);
  funcExit();
  add_entryToSTable("tmp___14", ret_SymValue, ret_ConValue, & tmp___14, 2);
  __cil_tmp52 = p[BB];
  addEntryToVariableTable("p[BB]", BB);
  add_entryToArraySTable("p", BB, "p210", & p[BB], & p[BB], 1);
  handleAssignmentSymbolically("__cil_tmp52", "p[BB]", & p[BB], & p[BB], 1);
  __cil_tmp53 = x - (double )1;
  handleAssignmentSymbolically("__cil_tmp53", "(- x 1)", & __cil_tmp53, & __cil_tmp53,
                               2);
  __cil_tmp54 = y - (double )1;
  handleAssignmentSymbolically("__cil_tmp54", "(- y 1)", & __cil_tmp54, & __cil_tmp54,
                               2);
  funcEntry("(int,grad_hash,variable,__cil_tmp52)#(double,grad_x,variable,__cil_tmp53)#(double,grad_y,variable,__cil_tmp54)#(double,grad_z,variable,z)",
            "grad_h grad_u grad_tmp grad_v grad_tmp___0 grad_tmp___1 grad_tmp___2 grad_tmp___3 grad___cil_tmp13",
            "grad");
  tmp___15 = grad(__cil_tmp52, __cil_tmp53, __cil_tmp54, z);
  funcExit();
  add_entryToSTable("tmp___15", ret_SymValue, ret_ConValue, & tmp___15, 2);
  __cil_tmp55 = p[AB];
  addEntryToVariableTable("p[AB]", AB);
  add_entryToArraySTable("p", AB, "p211", & p[AB], & p[AB], 1);
  handleAssignmentSymbolically("__cil_tmp55", "p[AB]", & p[AB], & p[AB], 1);
  __cil_tmp56 = y - (double )1;
  handleAssignmentSymbolically("__cil_tmp56", "(- y 1)", & __cil_tmp56, & __cil_tmp56,
                               2);
  funcEntry("(int,grad_hash,variable,__cil_tmp55)#(double,grad_x,variable,x)#(double,grad_y,variable,__cil_tmp56)#(double,grad_z,variable,z)",
            "grad_h grad_u grad_tmp grad_v grad_tmp___0 grad_tmp___1 grad_tmp___2 grad_tmp___3 grad___cil_tmp13",
            "grad");
  tmp___16 = grad(__cil_tmp55, x, __cil_tmp56, z);
  funcExit();
  add_entryToSTable("tmp___16", ret_SymValue, ret_ConValue, & tmp___16, 2);
  funcEntry("(double,lerp_t,variable,u)#(double,lerp_a,variable,tmp___16)#(double,lerp_b,variable,tmp___15)",
            "lerp___cil_tmp4", "lerp");
  tmp___17 = lerp(u, tmp___16, tmp___15);
  funcExit();
  add_entryToSTable("tmp___17", ret_SymValue, ret_ConValue, & tmp___17, 2);
  __cil_tmp57 = p[BA];
  addEntryToVariableTable("p[BA]", BA);
  add_entryToArraySTable("p", BA, "p212", & p[BA], & p[BA], 1);
  handleAssignmentSymbolically("__cil_tmp57", "p[BA]", & p[BA], & p[BA], 1);
  __cil_tmp58 = x - (double )1;
  handleAssignmentSymbolically("__cil_tmp58", "(- x 1)", & __cil_tmp58, & __cil_tmp58,
                               2);
  funcEntry("(int,grad_hash,variable,__cil_tmp57)#(double,grad_x,variable,__cil_tmp58)#(double,grad_y,variable,y)#(double,grad_z,variable,z)",
            "grad_h grad_u grad_tmp grad_v grad_tmp___0 grad_tmp___1 grad_tmp___2 grad_tmp___3 grad___cil_tmp13",
            "grad");
  tmp___18 = grad(__cil_tmp57, __cil_tmp58, y, z);
  funcExit();
  add_entryToSTable("tmp___18", ret_SymValue, ret_ConValue, & tmp___18, 2);
  __cil_tmp59 = p[AA];
  addEntryToVariableTable("p[AA]", AA);
  add_entryToArraySTable("p", AA, "p213", & p[AA], & p[AA], 1);
  handleAssignmentSymbolically("__cil_tmp59", "p[AA]", & p[AA], & p[AA], 1);
  funcEntry("(int,grad_hash,variable,__cil_tmp59)#(double,grad_x,variable,x)#(double,grad_y,variable,y)#(double,grad_z,variable,z)",
            "grad_h grad_u grad_tmp grad_v grad_tmp___0 grad_tmp___1 grad_tmp___2 grad_tmp___3 grad___cil_tmp13",
            "grad");
  tmp___19 = grad(__cil_tmp59, x, y, z);
  funcExit();
  add_entryToSTable("tmp___19", ret_SymValue, ret_ConValue, & tmp___19, 2);
  funcEntry("(double,lerp_t,variable,u)#(double,lerp_a,variable,tmp___19)#(double,lerp_b,variable,tmp___18)",
            "lerp___cil_tmp4", "lerp");
  tmp___20 = lerp(u, tmp___19, tmp___18);
  funcExit();
  add_entryToSTable("tmp___20", ret_SymValue, ret_ConValue, & tmp___20, 2);
  funcEntry("(double,lerp_t,variable,v)#(double,lerp_a,variable,tmp___20)#(double,lerp_b,variable,tmp___17)",
            "lerp___cil_tmp4", "lerp");
  tmp___21 = lerp(v, tmp___20, tmp___17);
  funcExit();
  add_entryToSTable("tmp___21", ret_SymValue, ret_ConValue, & tmp___21, 2);
  funcEntry("(double,lerp_t,variable,w)#(double,lerp_a,variable,tmp___21)#(double,lerp_b,variable,tmp___14)",
            "lerp___cil_tmp4", "lerp");
  tmp___22 = lerp(w, tmp___21, tmp___14);
  funcExit();
  add_entryToSTable("tmp___22", ret_SymValue, ret_ConValue, & tmp___22, 2);
  __cil_tmp62 = isNotQueueEmpty();
  if (__cil_tmp62) {
    enQueue();
    directPathConditions();
    delete_allSTableEntry();
    delete_allStructTableEntry();
    noise(p, permutation, x, y, z);
  } else {
    __cil_tmp62 = startCDG();
    add_entryToSTable("__cil_tmp62", "Function", & __cil_tmp62, & __cil_tmp62, 1);
    if (__cil_tmp62) {
      __cil_tmp62 = getTestCases();
      noise(p, permutation, x, y, z);
    }
  }
  return (tmp___22);
...
static int init(void)
{
  ...
  {
  init_i___0 = 0;
  add_entryToSTable("init_i___0", "Constant", & init_i___0, & init_i___0, 1);
  init_i___0 = 0;
  add_entryToSTable("init_i___0", "Constant", & init_i___0, & init_i___0, 1);
  {
  if (init_i___0 < 256) {
    init_tmp = permutation[init_i___0];
    addEntryToVariableTable("permutation[init_i___0]", init_i___0);
    add_entryToArraySTable("permutation", init_i___0, "permutation214", & permutation[init_i___0],
                           & permutation[init_i___0], 1);
    handleAssignmentSymbolically("init_tmp", "permutation[init_i___0]", & permutation[init_i___0],
                                 & permutation[init_i___0], 1);
    p[init_i___0] = init_tmp;
    handleArraySymbolically("p", init_i___0, "init_tmp", & p[init_i___0], & p[init_i___0],
                            1);
    p[256 + init_i___0] = init_tmp;
    handleArraySymbolically("p", 256 + init_i___0, "init_tmp", & p[256 + init_i___0],
                            & p[256 + init_i___0], 1);
    init_i___0 ++;
    handleAssignmentSymbolically("init_i___0", "(+ init_i___0 1)", & init_i___0, & init_i___0,
                                 1);
  }
  if (init_i___0 < 256) {
    init_tmp = permutation[init_i___0];
    addEntryToVariableTable("permutation[init_i___0]", init_i___0);
    add_entryToArraySTable("permutation", init_i___0, "permutation215", & permutation[init_i___0],
                           & permutation[init_i___0], 1);
    handleAssignmentSymbolically("init_tmp", "permutation[init_i___0]", & permutation[init_i___0],
                                 & permutation[init_i___0], 1);
    p[init_i___0] = init_tmp;
  ...
  {
  init___cil_tmp3 = 0;
  add_entryToSTable("init___cil_tmp3", "Constant", & init___cil_tmp3, & init___cil_tmp3,
                    1);
  {
  mapConcolicValues("init___cil_tmp3", & init___cil_tmp3);
  return (init___cil_tmp3);
  }
  }
}
}
...
void callInstrumentedFun(void)
{
  ...
  {
  enQueue();
  noise(argvar.p, argvar.permutation, argvar.x, argvar.y, argvar.z);
}
}
void main(void)
{
  ...
  while (1) {
    if (__cil_tmp8 >= 512) {
      ...
      __cil_tmp7 = rand();
      argvar.p[__cil_tmp8] = __cil_tmp7 % 20;
      __cil_tmp8 ++;
    }
  }
  __cil_tmp6 = 0;
  while (1) {
    if (__cil_tmp6 >= 256) {
      ...
      __cil_tmp5 = rand();
      argvar.permutation[__cil_tmp6] = __cil_tmp5 % 20;
      __cil_tmp6 ++;
    }
  }
  __cil_tmp4 = rand();
  argvar.x = __cil_tmp4 % 20;
  __cil_tmp3 = rand();
  argvar.y = __cil_tmp3 % 20;
  __cil_tmp2 = rand();
  argvar.z = __cil_tmp2 % 20;
  initSID();
  isCopyOfHolder();
  createCDG();
  createSidTable();
