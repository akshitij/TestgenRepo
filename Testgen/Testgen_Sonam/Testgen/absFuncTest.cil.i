...
#pragma merger("0","./absFuncTest.i","-g,-g")
...
int foo(int foo_n )
{
  ...
  if (foo_n < 0) {
    ...
    foo_n = foo_n;
    handleAssignmentSymbolically("foo_n", "foo_n", & foo_n, & foo_n, 1);
  }
  {
...
void createSidTable(void)
{
  ...
  {
  add_condition(6, "(>= a 0)", "(not (>= a 0))", 0, 0);
}
}
struct arguments {
   int a ;
...
int main1(int a )
{
  ...
  printTestCase("absFuncTest_main1_1433686019.tc", __cil_tmp7);
  ...
  {
  exp_outcome = a >= 0;
  handleAssignmentSymbolically("exp_outcome", "(>= a 0)", & exp_outcome, & exp_outcome,
                               1);
  overall_outcome = (int )getConditionalOutcome(6, exp_outcome);
  if (overall_outcome) {
    setBranchInfo(6, 1, 0);
    setTrueExpr(6, "(>= a 0)");
    setFalseExpr(6, "(not (>= a 0))");
    addToTree(6, 1, "(>= a 0)", "(not (>= a 0))", 0, 1);
    delete_allVariableTableEntry();
    b = 1;
