...
#pragma merger("0","./tripleFloat.i","-g,-g")
...
void createSidTable(void)
{
  ...
  {
  add_condition(2, "(>= a 15)", "(not (>= a 15))", 0, 0);
}
}
struct arguments {
   float a ;
...
int main1(float a )
{
  ...
  printTestCase("tripleFloat_main1_1435768880.tc", __cil_tmp7);
  add_entryToSTable("a", "s0", & a, & a, 2);
  printf((char const * __restrict )"a = %f\n", (double )a);
  a = (float )((double )a * 3.3);
  handleAssignmentSymbolically("a", "(* a 3.3)", & a, & a, 2);
  {
  exp_outcome = a >= (float )15;
  handleAssignmentSymbolically("exp_outcome", "(>= a 15)", & exp_outcome, & exp_outcome,
                               1);
  overall_outcome = (int )getConditionalOutcome(2, exp_outcome);
  if (overall_outcome) {
    setBranchInfo(2, 1, 0);
    setTrueExpr(2, "(>= a 15)");
    setFalseExpr(2, "(not (>= a 15))");
    addToTree(2, 1, "(>= a 15)", "(not (>= a 15))", 0, 1);
    delete_allVariableTableEntry();
    b = (float )1.1;
    ...
    setTrueExpr(2, "(>= a 15)");
    setFalseExpr(2, "(not (>= a 15))");
    addToTree(2, 1, "(>= a 15)", "(not (>= a 15))", 0, 0);
    delete_allVariableTableEntry();
    b = (float )(- 1.1);
