...
#pragma merger("0","./checkGlobal.i","-g,-g")
...
void createSidTable(void)
{
  ...
  {
  add_condition(1, "(> asdf 0)", "(not (> asdf 0))", 0, 0);
}
}
struct arguments {
   int asdf ;
...
int foo(int global_asdf )
{
  ...
  printTestCase("checkGlobal_foo_1435445674.tc", __cil_tmp6);
  add_entryToSTable("asdf", "s0", & asdf, & asdf, 1);
  {
  exp_outcome = asdf > 0;
  handleAssignmentSymbolically("exp_outcome", "(> asdf 0)", & exp_outcome, & exp_outcome,
                               1);
  overall_outcome = (int )getConditionalOutcome(1, exp_outcome);
  if (overall_outcome) {
    setBranchInfo(1, 1, 0);
    setTrueExpr(1, "(> asdf 0)");
    setFalseExpr(1, "(not (> asdf 0))");
    addToTree(1, 1, "(> asdf 0)", "(not (> asdf 0))", 0, 1);
    delete_allVariableTableEntry();
    j = 1;
    ...
    setTrueExpr(1, "(> asdf 0)");
    setFalseExpr(1, "(not (> asdf 0))");
    addToTree(1, 1, "(> asdf 0)", "(not (> asdf 0))", 0, 0);
    delete_allVariableTableEntry();
    j = -1;
    add_entryToSTable("j", "Constant", & j, & j, 1);
  }
...
int main1(void)
{
  ...
  {
  scanf((char const * __restrict )"%d", & i___0);
  if (i___0 > 10) {
    j = 2;
  } else {
