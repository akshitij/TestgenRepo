...
let copy_var =
  ...
  fun () ->
    decr counter;
    fun vi ->
      let vi' = copyVarinfo vi in
      (*vi'.vname <- vi.vname ^ "_unroll_" ^ (mkString (- !counter)); 
...
let loop1  (f: file) : unit =
 let k = read_int () in
  ...
  let doGlobal = function
   ...
   | GFun (fdec, loc)  ->
  let loop1Visitor = new loop1VisitorClass fdec in
        ignore (visitCilFunction loop1Visitor fdec);
