...
#pragma merger("0","./tripleVerifyFunc.i","-g,-g")
...
int foo(int foo_n )
{
  ...
  {
  foo_n = 3 * foo_n;
  handleAssignmentSymbolically("foo_n", "(* 3 foo_n)", & foo_n, & foo_n, 1);
  {
...
void createSidTable(void)
{
  ...
  {
  add_condition(4, "(= a 45)", "(not (= a 45))", 0, 0);
}
}
struct arguments {
   int a ;
...
int main1(int a )
{
  ...
  if (overall_outcome) {
    setBranchInfo(4, 1, 0);
    setTrueExpr(4, "(= a 45)");
    setFalseExpr(4, "(not (= a 45))");
    addToTree(4, 1, "(= a 45)", "(not (= a 45))", 0, 1);
    delete_allVariableTableEntry();
    b = 1;
    ...
    setTrueExpr(4, "(= a 45)");
    setFalseExpr(4, "(not (= a 45))");
    addToTree(4, 1, "(= a 45)", "(not (= a 45))", 0, 0);
    delete_allVariableTableEntry();
    b = -1;
    add_entryToSTable("b", "Constant", & b, & b, 1);
  }
