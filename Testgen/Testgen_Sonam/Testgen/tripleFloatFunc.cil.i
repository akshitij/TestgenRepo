...
#pragma merger("0","./tripleFloatFunc.i","-g,-g")
extern int scanf(char const * __restrict __format , ...) __asm__("__isoc99_scanf") ;
float foo(float foo_n )
{
  char *symName ;
  void *addr ;
  char in[15] ;
  ...
  {
  foo_n = (float )((double )foo_n * 3.3);
  handleAssignmentSymbolically("foo_n", "(* foo_n 3.3)", & foo_n, & foo_n, 2);
  {
  mapConcolicValues("foo_n", & foo_n);
  return (foo_n);
  }
}
}
...
void createSidTable(void)
{
  ...
  {
  add_condition(4, "(>= a 15.5)", "(not (>= a 15.5))", 0, 0);
}
}
struct arguments {
   float a ;
...
int main1(float a )
{
  ...
  sprintf(__cil_tmp7, "\t%f\n", a);
  printTestCase("tripleFloatFunc_main1_1434930599.tc", __cil_tmp7);
  add_entryToSTable("a", "s0", & a, & a, 2);
  funcEntry("(float,foo_n,variable,a)", "", "foo");
  a = foo(a);
  funcExit();
  add_entryToSTable("a", ret_SymValue, ret_ConValue, & a, 2);
  {
  exp_outcome = (double )a >= 15.5;
  handleAssignmentSymbolically("exp_outcome", "(>= a 15.5)", & exp_outcome, & exp_outcome,
                               1);
  overall_outcome = (int )getConditionalOutcome(4, exp_outcome);
  if (overall_outcome) {
    setBranchInfo(4, 1, 0);
    setTrueExpr(4, "(>= a 15.5)");
    setFalseExpr(4, "(not (>= a 15.5))");
    addToTree(4, 1, "(>= a 15.5)", "(not (>= a 15.5))", 0, 1);
    delete_allVariableTableEntry();
    b = (float )1.1;
    ...
    setTrueExpr(4, "(>= a 15.5)");
    setFalseExpr(4, "(not (>= a 15.5))");
    addToTree(4, 1, "(>= a 15.5)", "(not (>= a 15.5))", 0, 0);
    delete_allVariableTableEntry();
    b = (float )(- 1.1);
    handleAssignmentSymbolically("b", "(- 1.1)", & b, & b, 2);
  }
  }
  {
  __cil_tmp3 = 0;
