...
#pragma merger("0","./simpleif.i","-g,-g")
extern int ( scanf)() ;
...
void createSidTableOf_main1(void)
{
  ...
  {
  add_condition(2, "(> a 2)", "(not (> a 2))", 0, 0);
}
}
struct arguments {
   int a ;
...
int main1(int a )
{
  ...
  printTestCase("simpleif_main1_1429697065.tc", __cil_tmp5);
  add_entryToSTable("a", "s0", & a, & a, 1);

  {
  exp_outcome = a > 2;
  handleAssignmentSymbolically("exp_outcome", "(> a 2)", & exp_outcome, & exp_outcome,
                               1);
  overall_outcome = (int )getConditionalOutcome(2, exp_outcome);
  if (overall_outcome) {
    setBranchInfo(2, 1, 0);
    setTrueExpr(2, "(> a 2)");
    setFalseExpr(2, "(not (> a 2))");
    addToTree(2, 1, "(> a 2)", "(not (> a 2))", 0, 1);
    delete_allVariableTableEntry();
    a = 3;
    add_entryToSTable("a", "Constant", & a, & a, 1);
  } else {
    setBranchInfo(2, 0, 1);
    setTrueExpr(2, "(> a 2)");
    setFalseExpr(2, "(not (> a 2))");
    addToTree(2, 1, "(> a 2)", "(not (> a 2))", 0, 0);
    delete_allVariableTableEntry();
    a = 4;
    add_entryToSTable("a", "Constant", & a, & a, 1);
  }
