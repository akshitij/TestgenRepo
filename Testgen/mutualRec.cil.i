...
int kalle(int kalle_i___0 )
{
  ...
  if (kalle_i___0 <= 0) {
    ...
    kalle___cil_tmp4 = kalle_i___0 - 1;
    handleAssignmentSymbolically("kalle___cil_tmp4", "(- kalle_i___0 1)", & kalle___cil_tmp4,
                                 & kalle___cil_tmp4, 1);
    funcEntry("(int,anka_i___0,variable,kalle___cil_tmp4)", "anka_tmp anka___cil_tmp3 anka___cil_tmp4",
              "anka");
    kalle_tmp = anka(kalle___cil_tmp4);
    funcExit();
    add_entryToSTable("kalle_tmp", ret_SymValue, ret_ConValue, & kalle_tmp, 1);
    {
    mapConcolicValues("kalle_tmp", & kalle_tmp);
    return (kalle_tmp);
    }
  }
}
...
int anka(int anka_i___0 )
{
  ...
  if (anka_i___0 <= 0) {
    ...
    anka___cil_tmp4 = anka_i___0 - 1;
    handleAssignmentSymbolically("anka___cil_tmp4", "(- anka_i___0 1)", & anka___cil_tmp4,
                                 & anka___cil_tmp4, 1);
    funcEntry("(int,kalle_i___0,variable,anka___cil_tmp4)", "kalle_tmp kalle___cil_tmp3 kalle___cil_tmp4",
              "kalle");
    anka_tmp = kalle(anka___cil_tmp4);
    funcExit();
    add_entryToSTable("anka_tmp", ret_SymValue, ret_ConValue, & anka_tmp, 1);
    {
    mapConcolicValues("anka_tmp", & anka_tmp);
    return (anka_tmp);
    }
  }
}
...
void createSidTable(void)
{
  ...
  {
  add_condition(2, "(> res 10)", "(not (> res 10))", 0, 0);
}
}
struct arguments {
   int In ;
...
void main1(int In )
{
  ...
  printTestCase("mutualRec_main1_1435879426.tc", __cil_tmp7);
  add_entryToSTable("In", "s0", & In, & In, 1);
  funcEntry("(int,fib_i___0,variable,In)", "fib_tmp fib_tmp___0 fib___cil_tmp4 fib___cil_tmp5 fib___cil_tmp6 fib___cil_tmp7 fib___cil_tmp8",
            "fib");
  tmp = fib(In);
  funcExit();
  add_entryToSTable("tmp", ret_SymValue, ret_ConValue, & tmp, 1);
  res = tmp;
  handleAssignmentSymbolically("res", "tmp", & tmp, & tmp, 1);
  {
  exp_outcome = res > 10;
  handleAssignmentSymbolically("exp_outcome", "(> res 10)", & exp_outcome, & exp_outcome,
                               1);
  overall_outcome = (int )getConditionalOutcome(2, exp_outcome);
  if (overall_outcome) {
    setBranchInfo(2, 1, 0);
    setTrueExpr(2, "(> res 10)");
    setFalseExpr(2, "(not (> res 10))");
    addToTree(2, 1, "(> res 10)", "(not (> res 10))", 0, 1);
    delete_allVariableTableEntry();
    printf((char const * __restrict )"For In = %d res = %d\n", In, res);
  } else {
    setBranchInfo(2, 0, 1);
    setTrueExpr(2, "(> res 10)");
    setFalseExpr(2, "(not (> res 10))");
    addToTree(2, 1, "(> res 10)", "(not (> res 10))", 0, 0);
    delete_allVariableTableEntry();
    printf((char const * __restrict )"For In = %d res = %d\n", In, res);
  }
  }
