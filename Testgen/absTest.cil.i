...
void createSidTable(void)
{
  ...
  add_condition(5, "(>= a 0)", "(not (>= a 0))", 0, 0);
}
}
struct arguments {
   int a ;
...
int main1(int a )
{
  ...
  printTestCase("absTest_main1_1433094592.tc", __cil_tmp6);
  ...
  if (overall_outcome) {
    ...
    delete_allVariableTableEntry();
    a = a;
    handleAssignmentSymbolically("a", "a", & a, & a, 1);
  }
  }
  {
  exp_outcome = a >= 0;
  ...
  if (overall_outcome) {
    setBranchInfo(5, 1, 0);
    setTrueExpr(5, "(>= a 0)");
    setFalseExpr(5, "(not (>= a 0))");
    addToTree(5, 1, "(>= a 0)", "(not (>= a 0))", 0, 1);
    delete_allVariableTableEntry();
    b = 1;
