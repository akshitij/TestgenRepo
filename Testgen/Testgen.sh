...
bin/cilly  --noPrintLn --doParam --dohandleBreak --dohandleAssert --dotestgenStub --param_file=$file_name --dohandleCalls --doinsertTestDriver --dohandleExp --domodifyVars --doloopUnroll --domakeCDG --doloopUnrollOthers --doextractStruct --dosidTable --dolevel_struct --dovartype --dovarUpdated --dohelperpathsym --dopathSymbol --doinsertReturnHolders --doinstMapConcolic --doentryExit --dofixMainForGlobals -g -lm --save-temps --merge src/src/utils.c src/src/sidTable.c src/src/directAndSolve.c src/src/symbolicExec.c src/src/queue.c src/src/levelTree.c src/src/stringTokenize.c src/src/arrayAndPointersSymbolicExec.c src/src/updateIntegerValues.c src/src/flatds.c src/src/cdg.c src/src/cdgWrapper.c  src/src/stack.c src/src/ipaRecursive.c $base -g -o $file_name.out
